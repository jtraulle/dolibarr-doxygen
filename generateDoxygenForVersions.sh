...
while IFS= read -r version; do
  ...
  echo ""
  docker run --user ${UID} --rm -v `pwd`:/home jtraulle/doxygen:1.8.16 /bin/sh -c "( cat dolibarr.doxyfile ; echo 'PROJECT_NUMBER=$version' ; echo 'OUTPUT_DIRECTORY=build_$VERSIONSFILE' ; echo 'GENERATE_HTML=NO' ; echo 'GENERATE_LATEX=YES' ; echo 'LATEX_OUTPUT=pdf_$version' ) | doxygen -"
